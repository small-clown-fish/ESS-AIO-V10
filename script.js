...
function renderProjectConfig(){
  ...
  $('projectBmsRows').innerHTML=bms.map(d=>{ const name=String(d.name||''); return `<tr><td>${esc(name)}</td><td>${esc(d.host)}</td><td>${esc(d.port)}</td><td>${esc(d.unit_id)}</td><td>${esc(d.interval||d.poll_interval)}</td><td>${esc(d.profile||d.driver||'')}</td><td><button onclick="fillBmsConfig(decArg('${encArg(name)}'))">Edit</button> <button class="danger" onclick="removeBmsByName(decArg('${encArg(name)}'))">Remove</button></td></tr>`; }).join('') || '<tr><td colspan="7" class="muted">No BMS configured</td></tr>';
  $('projectPcsRows').innerHTML=Object.entries(pcs).sort().map(([name,c])=>`<tr><td>${esc(name)}</td><td>${esc(c.host)}</td><td>${esc(c.port)}</td><td>${esc(c.unit_id)}</td><td>${esc(c.profile||c.driver||'')}</td><td>${esc(c.enabled)}</td><td><button onclick="fillPcsConfig(decArg('${encArg(name)}'))">Edit</button> <button class="danger" onclick="removePcsByName(decArg('${encArg(name)}'))">Remove</button></td></tr>`).join('') || '<tr><td colspan="7" class="muted">No PCS configured</td></tr>';
...
function chipHtml(clusterName, kind, values){
  return (values||[]).map(v=>`<span class="chip">${esc(v)} <button title="Remove" onclick="removeBindChip(decArg('${encArg(clusterName)}'),decArg('${encArg(kind)}'),decArg('${encArg(v)}'))">×</button></span>`).join('') || '<span class="muted">None</span>';
}
  ...
  const rows=(clusters||[]).map((c,idx)=>{
    ...
    return `<tr data-cluster="${esc(name)}">
      ...
      <td>
        ...
        <div class="bind-picker"><select id="bindBmsPick_${cid}" onfocus="touchClusterBindingEditor(60000)" onchange="touchClusterBindingEditor(60000)"></select><button onmousedown="touchClusterBindingEditor(60000)" onclick="addBindPick(decArg('${encArg(name)}'),'bms')">Add</button></div>
      </td>
      <td>
        <div id="bindPcsChips_${cid}" class="chip-list"></div>
        <div class="bind-picker"><select id="bindPcsPick_${cid}" onfocus="touchClusterBindingEditor(60000)" onchange="touchClusterBindingEditor(60000)"></select><button onmousedown="touchClusterBindingEditor(60000)" onclick="addBindPick(decArg('${encArg(name)}'),'pcs')">Add</button></div>
      </td>
      <td>
        <code id="clusterRowPower_${cid}" class="power-map-code" title="Click Edit to configure power map">{}</code>
        <div class="row-actions"><button onclick="editRowPowerMapByName(decArg('${encArg(name)}'))">Edit</button><button onclick="autoEvenPowerMapForCluster(decArg('${encArg(name)}'))">Auto Even</button><button class="danger" onclick="clearPowerMapAndSave(decArg('${encArg(name)}'))">Clear Power Map</button></div>
      </td>
      <td><button onclick="saveClusterBindingByName(decArg('${encArg(name)}'))">Save</button> <button class="danger" onclick="removeClusterBindingRow(decArg('${encArg(name)}'))">Remove Cluster</button></td>
    </tr>`;
  }).join('');
